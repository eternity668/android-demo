...
public class VideoActivity extends Activity
{
	...
	actionBar.setTitle(intent.getStringExtra(resources
	...
	curCatalogId = intent.getStringExtra(resources
		.getString(R.string.catalogId));
	...
	videoListView.setOnRefreshListener(new OnRefreshListener() {
		...
		List<VideoInfo> videoInfos = videoListAdapter
			.getVideoInfoList();
		...
		new AsyncTask<String, Void, List<VideoInfo>>() {
			...
			try
			{
				...
				videoInfo.setSize(resources
					.getString(R.string.emptyString));
				String status = jsonObject.getString("status");
				status = StatusTransition.toChinese(status,
					resources);
				videoInfo.setStatus(status);
				if (status.equals(resources
					.getString(R.string.AUDIT_SUCCESS)))
				{
			...
			{
			    videoEmpTextView
				    .setText(R.string.videoListLoadError);
			}
	...
	{
	    Toast.makeText(this, resources.getString(R.string.isRefresh),
		    Toast.LENGTH_LONG).show();
	    return;
	}
